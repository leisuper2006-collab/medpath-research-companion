...
# Run locally; do not upload private data to a static website.`;
  ...
  function render() {
    ...
    if (!(path === "/plot-gallery" || path === "/plot-studio" || path.startsWith("/plot-gallery/") || path.startsWith("/plot-run/"))) {
      document.body.classList.remove("medpath-r133-plot");
      return false;
    }
    ...
    let html = "";
    if (path === "/plot-gallery" || path === "/plot-studio") html = galleryPage();
  ...
  function start() {
    ...
    if ("MutationObserver" in window) {
      const obs = new MutationObserver(() => {
        const path = route();
        const app = document.getElementById("app");
        const isPlotRoute = path === "/plot-gallery" || path === "/plot-studio" || path.startsWith("/plot-gallery/") || path.startsWith("/plot-run/");
        const wasOverwritten = app && app.getAttribute("data-round133-owned") === VERSION && !app.querySelector(".r133-shell");
        if (isPlotRoute && (app?.getAttribute("data-round133-owned") !== VERSION || wasOverwritten)) {
          render();
        }
      });
