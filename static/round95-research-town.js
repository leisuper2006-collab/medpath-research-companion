(() => {
  ...
  function shell() {
    return `
      <div class="r77-town is-concept-mode" data-r77-root>
        ...
        <button class="r77-panel-peek r77-panel-peek-left" type="button" data-r77-action="toggle-left"><span>榜</span><b>贡献榜</b></button>
        <aside class="r77-left-panel" data-r77-left-panel>
          <div class="r77-left-head"><b>科研贡献榜</b><button class="r77-panel-toggle" type="button" data-r77-action="toggle-left" aria-label="收起贡献榜">收起</button></div>
        ...
        <button class="r77-panel-peek r77-panel-peek-right" type="button" data-r77-action="toggle-right"><span>☰</span><b>快捷</b></button>
        <nav class="r77-rail" data-r77-rail><button class="r77-rail-collapse" data-r77-action="toggle-right" type="button"><span>×</span><small>收起</small></button><button data-r77-action="tasks"><span>★</span><small>任务</small></button><button data-r77-action="catalog"><span>▣</span><small>背包</small></button><button data-r77-action="skill"><span>⚡</span><small>技能</small></button><button data-r77-action="cases"><span>▤</span><small>案例</small></button><button data-r77-action="tools"><span>⌘</span><small>工具</small></button><button data-r77-action="decor"><span>♣</span><small>装饰</small></button><button data-r77-action="rank"><span>☺</span><small>好友</small></button><button data-r77-action="achieve"><span>🏆</span><small>成就</small></button></nav>
        ...
        <div class="r77-concept-toast" data-r77-toast>${esc(state.toast)}</div>
        ...
        <button class="r77-dock-peek" type="button" data-r77-action="toggle-bottom"><span>🔨</span><b>建造</b></button>
        <div class="r77-dock" data-r77-dock><button class="r77-dock-collapse" data-r77-action="toggle-bottom" type="button"><span>⌄</span><small>收起</small></button><button data-r77-action="explore" onclick="window.medpathR77Action && window.medpathR77Action('explore')"><span>◉</span><small>探索</small></button><button data-r77-action="build" onclick="window.medpathR77Action && window.medpathR77Action('build')"><span>🔨</span><small>建造</small></button><button data-r77-action="decor" onclick="window.medpathR77Action && window.medpathR77Action('decor')"><span>♣</span><small>装饰</small></button><button data-r77-action="shop" onclick="window.medpathR77Action && window.medpathR77Action('shop')"><span>▥</span><small>商店</small></button><button data-r77-action="warehouse" onclick="window.medpathR77Action && window.medpathR77Action('warehouse')"><span>▤</span><small>仓库</small></button><button data-r77-action="map" onclick="window.medpathR77Action && window.medpathR77Action('map')"><span>▧</span><small>地图</small></button></div>
        ...
        <section class="r77-concept-drawer" data-r77-drawer>
          <header>
            ...
            <div class="r77-drawer-actions">
              <button data-r77-drawer-minimize type="button" aria-label="收起建筑商店">－</button>
              <button data-r77-close type="button" aria-label="关闭建筑商店">×</button>
            </div>
          </header>
  ...
  function renderDialog() {
    ...
    dialog.querySelector("[data-r77-build]")?.addEventListener("click", () => {
      state.points += 40;
      logTask(`把${b.name}加入常用建筑`, 40, b.id);
      state.toast = `已将“${b.name}”加入常用建筑。后续支持拖拽摆放、皮肤和他人拜访。`;
      persist();
      sync();
    });
    dialog.querySelector("[data-r77-drawer-open]")?.addEventListener("click", () => {
      state.drawer = true;
      state.toast = "建筑库已打开：这里会扩展到100+建筑，支持绑定自己的Skill和科研工作流。";
      sync();
    });
    movePlayerTo(b);
  }
  ...
  function submitTask(form) {
    ...
    const id = routeTask(text);
    input.value = "";
    state.toast = `AI小镇助手已把需求分流到“${hotspots.find((b) => b.id === id)?.name}”：${text}`;
    select(id);
    return false;
  }
  ...
  function attachEvents() {
    ...
    r.querySelectorAll("[data-r77-hotspot],[data-r77-label]").forEach((button) => {
      const id = button.dataset.r77Hotspot || button.dataset.r77Label;
  ...
  function mount() {
    ...
    if (!stage || stage.dataset.r77Mounted === "1") return;
  ...
  function resetForRoute() {
    ...
    const stage = document.getElementById("r73-island-stage");
    if (stage && stage.dataset.r77Mounted !== "1") mount();
  }
