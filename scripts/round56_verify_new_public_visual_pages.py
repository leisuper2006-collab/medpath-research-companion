...
def check_source_page(page, source_id: str, screenshot_name: str) -> dict:
    page.goto(f"{BASE}/?fresh=round56source#/source-library/{source_id}", wait_until="domcontentloaded")
    ...
    return {
        ...
        "images": images,
        "has_round56_asset": any("round56_mutation_type_distribution" in (img.get("src") or "") for img in images),
...
def main() -> None:
    targets = [
        ("aml_ohsu_2022", "round56_aml_source_visual_verified.png"),
        ("bladder_columbia_msk_2018", "round56_bladder_source_visual_verified.png"),
        ("breast_msk_2018", "round56_breast_source_visual_verified.png"),
    ]
    ...
    failed = [
        item
        for item in results
        if not item["visual_section_present"]
        or item["visual_cards"] < 1
        or item["good_image_count"] < 1
        or not item["has_round56_asset"]
    ...
    if failed:
        raise SystemExit(f"Round56 public visual page verification failed: {failed}")
