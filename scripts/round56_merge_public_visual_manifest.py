...
NEW = ROOT / "data" / "public_reproducible_examples" / "round56_next_public_visual_manifest.json"
REPORT = ROOT / "docs" / "round56_public_visual_coverage_report.md"
...
def main() -> None:
    ...
    source_title = {x["id"]: x.get("title", "") for x in public_sources}
    ...
    lines.extend(["", "## 仍待补充的前 30 个来源", ""])
    for sid in uncovered_public[:30]:
        lines.append(f"- `{sid}`：{source_title.get(sid, '')}")
    lines.extend(
        [
            "",
            "## 真实性边界",
