...
gene_catalog <- tibble::tribble(
  ...
  "JAK2", 3717L, "NOTCH1", 4851L, "SF3B1", 23451L
)

public_sources <- fromJSON(file.path(root, "data", "public_example_sources.json"), simplifyVector = TRUE)
public_sources <- as_tibble(public_sources)
...
fetch_mutations <- function(study_id) {
  ...
  raw_path <- file.path(data_dir, sprintf("%s_round56_selected_gene_mutations_raw.json", study_id))
...
build_plot <- function(study_id) {
  ...
  fetched <- fetch_mutations(study_id)
  ...
  summary_path <- file.path(data_dir, sprintf("%s_round56_mutation_type_summary.csv", study_id))
  metadata_path <- file.path(data_dir, sprintf("%s_round56_mutation_example_metadata.json", study_id))
  svg_path <- file.path(out_dir, sprintf("%s_round56_mutation_type_distribution.svg", study_id))
  png_path <- file.path(out_dir, sprintf("%s_round56_mutation_type_distribution.png", study_id))
  ...
  p <- ggplot(plot_df, aes(x = mutation_records, y = gene, fill = mutationType)) +
    ...
    labs(
      ...
      y = NULL,
      caption = "Synthetic teaching re-plot from public API metadata/records. For education and research training only; not for clinical diagnosis."
    ) +
  ...
  metadata <- list(
    id = paste0(study_id, "_round56_mutation_type_distribution"),
    ...
    script = "scripts/round56_reproduce_cbioportal_next_public_plots.R",
...
manifest_path <- file.path(data_dir, "round56_next_public_visual_manifest.json")
skipped_path <- file.path(data_dir, "round56_next_public_visual_skipped.json")
write_json(results, manifest_path, auto_unbox = TRUE, pretty = TRUE)
write_json(skipped, skipped_path, auto_unbox = TRUE, pretty = TRUE)

message("Round56 generated: ", length(results), " visuals; skipped: ", length(skipped))
if (length(skipped)) {
